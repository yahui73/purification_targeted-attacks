...
class Empirical():
    ...
    def run(self, log_progress):
        ...
        if self.config.structure.dataset == 'CIFAR10-C':
            ...
            testLoader_list = importData(dataset=self.config.structure.dataset, train=False, shuffle=False,
            ...
            testLoader = torch.utils.data.DataLoader(image_datasets, batch_size=self.config.structure.bsize,
                                                     shuffle=True, num_workers=64)
        elif self.config.structure.dataset == 'ImageNet' and self.config.attack.attack_method == 'bpda_strong':
            testLoader = importData(dataset=self.config.structure.dataset, train=False, shuffle=True,
                                    bsize=self.config.structure.bsize)
        ...
        #     testLoader = importData(dataset=self.config.structure.dataset, train=False, shuffle=True, bsize=self.config.structure.bsize)
            ...
            testLoader = importData(dataset=self.config.structure.dataset, train=False, shuffle=False,
                                    bsize=self.config.structure.bsize)
        ...
        if self.config.structure.dataset in ["TinyImageNet", "ImageNet", "ImageNet-C"]:
        ...
        if self.config.structure.dataset in ["CIFAR10", "CIFAR10-C",
                                             "CIFAR100"]:  # CIFAR10 setting, trained by WideResNet
            ...
            # network_clf = torch.hub.load('pytorch/vision:v0.10.0', 'wide_resnet50_2', pretrained=True)  #wxz
            # network_clf = network_clf.to(self.config.device.clf_device)   #wxz
            network_clf = torchvision.models.resnet50(pretrained=True).to(self.config.device.clf_device)  # wxz
        ...
        elif self.config.structure.dataset in ["TinyImageNet", "ImageNet", "ImageNet-C"]:
        ...
        # Run
        for i, (x, y) in enumerate(tqdm.tqdm(testLoader)):  # show jindutiao
            ...
            print(self.config.structure.dataset)
            ...
            if self.config.attack.if_attack:
                if self.config.structure.dataset in ["CIFAR10", "CIFAR10-C"]:
                    x_adv, success, acc = eval(self.config.attack.attack_method)(x, y, diffusion, network_clf,
                                                                                 self.config)
                elif self.config.structure.dataset in ["TinyImageNet", "ImageNet"]:
                    x_adv, success, acc = eval(self.config.attack.attack_method)(x, y, diffusion, network_clf,
                                                                                 self.config, model=model)
                attack_time = elapsed_seconds(start_time, datetime.now())
                print("[{}] Epoch {}:\t{:.2f} seconds to attack {} data".format(str(datetime.now()), i, attack_time,
                                                                                self.config.structure.bsize))
            else:
                x_adv = x
                attack_time = 0.0
            ...
            x_pur_list_list = []
            start_time = datetime.now()
            print("[{}] Epoch {}:\tBegin purifying {} attacked images".format(str(datetime.now()), i,
                                                                              self.config.structure.bsize))
            ...
            print("[{}] Epoch {}:\t{:.2f} seconds to purify {} attacked images".format(str(datetime.now()), i,
                                                                                       purify_attacked_time,
                                                                                       self.config.structure.bsize))
            ...
            if self.config.purification.purify_natural:
                x_nat_pur_list_list = []
                start_time = datetime.now()
                print("[{}] Epoch {}:\tBegin purifying {} natural images".format(str(datetime.now()), i,
                                                                                 self.config.structure.bsize))
                for j in range(self.config.purification.path_number):
                    if self.config.structure.dataset in ["CIFAR10", "CIFAR10-C"]:
            ...
            with torch.no_grad():
                ...
                nat_correct = torch.eq(torch.argmax(y_t, dim=1),
                                       y.clone().to(self.config.device.clf_device)).float().sum()
                att_correct = torch.eq(torch.argmax(y_adv_t, dim=1),
                                       y.clone().to(self.config.device.clf_device)).float().sum()
            ...
            with torch.no_grad():
                start_time = datetime.now()
                print("[{}] Epoch {}:\tBegin predicting {} purified attacked images".format(str(datetime.now()), i,
                                                                                            self.config.structure.bsize))
                ...
                print(
                    "[{}] Epoch {}:\t{:.2f} seconds to predict {} purified attacked images".format(str(datetime.now()),
                                                                                                   i,
                                                                                                   classify_attacked_time,
                                                                                                   self.config.structure.bsize))

                # Classify all purified natural images
                if self.config.purification.purify_natural:
                    start_time = datetime.now()
                    print("[{}] Epoch {}:\tBegin predicting {} purified natural images".format(str(datetime.now()), i,
                                                                                               self.config.structure.bsize))
                    ...
                    print("[{}] Epoch {}:\t{:.2f} seconds to predict {} purified natural images".format(
                        str(datetime.now()), i, classify_natural_time, self.config.structure.bsize))
