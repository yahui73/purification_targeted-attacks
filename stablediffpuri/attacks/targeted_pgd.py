...
def targeted_pgd(x, y, diffusion, network_clf, config, model=None):
    ...
    if config.attack.ball_dim == -1:
        x_adv = projected_gradient_descent(network_clf, x, eps=config.attack.ptb / 255.,
                                           eps_iter=config.attack.ptb / 255. / 4, nb_iter=config.attack.attack_steps,
                                           norm=np.inf, clip_min=0., clip_max=1., y=y, targeted=True).to(
            config.device.clf_device)
